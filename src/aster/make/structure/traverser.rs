...
pub(super) fn traverse_import(
  ...
  let count = traverse_group(lazy, &import.source, &import.group, &mut stack)?;
...
fn traverse_group(
  ...
  for part in group.selectors.iter() {
    count += traverse_part(lazy, module, part, stack)?;
  };
...
fn traverse_part(
  ...
  match part {
    ...
    lang::module::import::ImportPart::Group(group) => traverse_group(lazy, module, group, stack),
    lang::module::import::ImportPart::Qualify(qualify) => {
      stack.push(qualify.name);

      let result = match &qualify.next {
        Some(next_part) => traverse_part(lazy, module, next_part, stack),
        None => {
