    ...
    for import in scope.imports.iter() {
      if import.name == index {
        return Ok(match import.reference.get()? {
          Some(weak) => ScopeSearch::Found(weak),
          None => {
            warn!(
              "{}: resolved reference to import, but import is unresolved",
              enchant!("search_in")
            );
            ScopeSearch::None
          },
    ...
    for export in scope.exports.iter() {
      if export.name.as_ref().is_some_and(|name| name == index) {
        return Ok(match export.reference.borrow().get()? {
          ...
          None => {
            warn!(
              "{}: resolved reference to export, but export is unresolved",
              enchant!("search_in")
            );
            ScopeSearch::None
          },
